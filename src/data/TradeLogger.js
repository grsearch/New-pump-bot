...
class TradeLogger {
  ...
  _initSchema() {
    ...
    this._ensureColumns('trades', [
      ...
      ['pool_base_amount_raw', 'TEXT'],
      ['pool_quote_amount_raw', 'TEXT'],
      ['pool_address', 'TEXT'],
      ['seller_tx', 'TEXT'],
      ['signal_price_before', 'REAL'],
      ['signal_pool_quote_sol', 'REAL'],
      ['signal_pool_base_amount_ui', 'REAL'],
      ['signal_raw_pool_quote_sol', 'REAL'],
      ['signal_virtual_quote_reserve_sol', 'REAL'],
      ['signal_effective_quote_reserve_sol', 'REAL'],
      ['signal_pool_base_amount_raw', 'TEXT'],
      ['signal_pool_quote_amount_raw', 'TEXT'],
      ['signal_virtual_quote_reserves_raw', 'TEXT'],
      ['signal_stream_region', 'TEXT'],
      ['signal_source', 'TEXT'],
      ['base_decimals', 'INTEGER'],
      ['signal_slot', 'INTEGER'],
      ['signal_transaction_index', 'INTEGER'],
      ['signal_ts', 'INTEGER'],
      ['signal_received_at', 'INTEGER'],
      ['latest_stream_slot_at_signal', 'INTEGER'],
      ['latest_stream_slot_at_order', 'INTEGER'],
      ['latest_stream_slot_at_quote', 'INTEGER'],
      ['slot_gap_at_signal', 'INTEGER'],
      ['slot_gap_at_quote', 'INTEGER'],
      ['rpc_context_slot', 'INTEGER'],
      ['rpc_pool_context_slot', 'INTEGER'],
      ['rpc_reserve_context_slot', 'INTEGER'],
      ['rpc_user_context_slot', 'INTEGER'],
      ['rpc_context_slot_approximate', 'INTEGER'],
      ['rpc_slot_gap_from_signal', 'INTEGER'],
      ['rpc_fetched_at_ms', 'INTEGER'],
      ['quote_started_at', 'INTEGER'],
      ['quote_ready_at', 'INTEGER'],
      ['quote_latency_ms', 'INTEGER'],
      ['signal_to_quote_ms', 'INTEGER'],
      ['stream_fast_path', 'INTEGER'],
      ['stream_signal_age_ms', 'INTEGER'],
      ['stream_slot_gap', 'INTEGER'],
      ['stream_state_fallback_reason', 'TEXT'],
...
${snapshotColumnsSql},
      ...
      insertTrade: this.db.prepare(`
        INSERT INTO trades
          (position_id, ts, mint, symbol, side, sol_amount, token_amount, price, signature,
           ...
           effective_quote_reserve_raw, pool_base_amount_raw, pool_quote_amount_raw,
           pool_address, seller_tx, signal_price_before, signal_pool_quote_sol,
           signal_pool_base_amount_ui, signal_raw_pool_quote_sol,
           signal_virtual_quote_reserve_sol, signal_effective_quote_reserve_sol,
           signal_pool_base_amount_raw, signal_pool_quote_amount_raw,
           signal_virtual_quote_reserves_raw,
           signal_stream_region, signal_source, base_decimals,
           signal_slot, signal_transaction_index, signal_ts, signal_received_at,
           latest_stream_slot_at_signal, latest_stream_slot_at_order,
           latest_stream_slot_at_quote, slot_gap_at_signal, slot_gap_at_quote,
           rpc_context_slot, rpc_pool_context_slot, rpc_reserve_context_slot,
           rpc_user_context_slot, rpc_context_slot_approximate,
           rpc_slot_gap_from_signal, rpc_fetched_at_ms, quote_started_at,
           quote_ready_at, quote_latency_ms, signal_to_quote_ms,
           stream_fast_path, stream_signal_age_ms, stream_slot_gap,
           stream_state_fallback_reason)
        VALUES (@positionId, @ts, @mint, @symbol, @side, @solAmount, @tokenAmount, @price, @signature,
                ...
                @effectiveQuoteReserveRaw, @poolBaseAmountRaw, @poolQuoteAmountRaw,
                @poolAddress, @sellerTx, @signalPriceBefore, @signalPoolQuoteSol,
                @signalPoolBaseAmountUi, @signalRawPoolQuoteSol,
                @signalVirtualQuoteReserveSol, @signalEffectiveQuoteReserveSol,
                @signalPoolBaseAmountRaw, @signalPoolQuoteAmountRaw,
                @signalVirtualQuoteReservesRaw,
                @signalStreamRegion, @signalSource, @baseDecimals,
                @signalSlot, @signalTransactionIndex, @signalTs, @signalReceivedAt,
                @latestStreamSlotAtSignal, @latestStreamSlotAtOrder,
                @latestStreamSlotAtQuote, @slotGapAtSignal, @slotGapAtQuote,
                @rpcContextSlot, @rpcPoolContextSlot, @rpcReserveContextSlot,
                @rpcUserContextSlot, @rpcContextSlotApproximate,
                @rpcSlotGapFromSignal, @rpcFetchedAtMs, @quoteStartedAt,
                @quoteReadyAt, @quoteLatencyMs, @signalToQuoteMs,
                @streamFastPath, @streamSignalAgeMs, @streamSlotGap,
                @streamStateFallbackReason)
      `),
      ...
      insertSwapEvent: this.db.prepare(`
        INSERT OR IGNORE INTO swap_events
          (ts, mint, symbol, signer, side, sol_volume, price, price_before, price_change_pct,
           slot, transaction_index, stream_region, signature, pool_address, pool_quote_after,
           source, price_reliable,
        ...
        VALUES
          (@ts, @mint, @symbol, @signer, @side, @solVolume, @price, @priceBefore, @priceChangePct,
           @slot, @transactionIndex, @streamRegion, @signature, @poolAddress, @poolQuoteAfter,
           @source, @priceReliable,
  ...
  logTrade({ positionId, ts, mint, symbol, side, solAmount, tokenAmount, price, signature,
             ...
             latestStreamSlotAtOrder, latestStreamSlotAtQuote, slotGapAtSignal,
             slotGapAtQuote, rpcContextSlot, rpcPoolContextSlot,
             rpcReserveContextSlot, rpcUserContextSlot,
             rpcContextSlotApproximate, rpcSlotGapFromSignal, rpcFetchedAtMs,
             quoteStartedAt, quoteReadyAt, quoteLatencyMs, signalToQuoteMs,
             streamFastPath, streamSignalAgeMs, streamSlotGap,
             streamStateFallbackReason }) {
    this.stmts.insertTrade.run({
      ...
      effectiveQuoteReserveRaw: effectiveQuoteReserveRaw || null,
      poolBaseAmountRaw: poolBaseAmountRaw || null,
      poolQuoteAmountRaw: poolQuoteAmountRaw || null,
      poolAddress: poolAddress || null,
      sellerTx: sellerTx || null,
      signalPriceBefore: signalPriceBefore ?? null,
      signalPoolQuoteSol: signalPoolQuoteSol ?? null,
      signalPoolBaseAmountUi: signalPoolBaseAmountUi ?? null,
      signalRawPoolQuoteSol: signalRawPoolQuoteSol ?? null,
      signalVirtualQuoteReserveSol: signalVirtualQuoteReserveSol ?? null,
      signalEffectiveQuoteReserveSol: signalEffectiveQuoteReserveSol ?? null,
      signalPoolBaseAmountRaw: signalPoolBaseAmountRaw || null,
      signalPoolQuoteAmountRaw: signalPoolQuoteAmountRaw || null,
      signalVirtualQuoteReservesRaw: signalVirtualQuoteReservesRaw || null,
      signalStreamRegion: signalStreamRegion || null,
      signalSource: signalSource || null,
      baseDecimals: baseDecimals ?? null,
      signalSlot: signalSlot ?? null,
      signalTransactionIndex: signalTransactionIndex ?? null,
      signalTs: signalTs ?? null,
      signalReceivedAt: signalReceivedAt ?? null,
      latestStreamSlotAtSignal: latestStreamSlotAtSignal ?? null,
      latestStreamSlotAtOrder: latestStreamSlotAtOrder ?? null,
      latestStreamSlotAtQuote: latestStreamSlotAtQuote ?? null,
      slotGapAtSignal: slotGapAtSignal ?? null,
      slotGapAtQuote: slotGapAtQuote ?? null,
      rpcContextSlot: rpcContextSlot ?? null,
      rpcPoolContextSlot: rpcPoolContextSlot ?? null,
      rpcReserveContextSlot: rpcReserveContextSlot ?? null,
      rpcUserContextSlot: rpcUserContextSlot ?? null,
      rpcContextSlotApproximate: rpcContextSlotApproximate ?? null,
      rpcSlotGapFromSignal: rpcSlotGapFromSignal ?? null,
      rpcFetchedAtMs: rpcFetchedAtMs ?? null,
      quoteStartedAt: quoteStartedAt ?? null,
      quoteReadyAt: quoteReadyAt ?? null,
      quoteLatencyMs: quoteLatencyMs ?? null,
      signalToQuoteMs: signalToQuoteMs ?? null,
      streamFastPath: streamFastPath ?? null,
      streamSignalAgeMs: streamSignalAgeMs ?? null,
      streamSlotGap: streamSlotGap ?? null,
      streamStateFallbackReason: streamStateFallbackReason || null,
    });
  }
  ...
  logSwapEvent(swap) {
    ...
    try {
      this.stmts.insertSwapEvent.run({
        ...
        slot: num(swap.slot),
        transactionIndex: num(swap.transactionIndex),
        streamRegion: swap.streamRegion || null,
