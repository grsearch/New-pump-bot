...
// Regression fixture from the SWOLEDOGE pricing incident.
const state = {
  poolBaseAmount: { toString: () => '100000000000000' },
  poolQuoteAmount: { toString: () => '135800000000' },
  pool: {
    virtualQuoteReserves: { toString: () => '17900000000' },
  },
};
...
assert.strictEqual(
  ...
  null,
  'missing virtual reserve must not fall back to raw-vault pricing',
);
...
detector.poolStateCache = { get: () => state, getAge: () => 0 };
detector._pricingRefreshRequestedAt = new Map();
...
let refreshCalls = 0;
const coldDetector = Object.create(DumpDetector.prototype);
coldDetector._pricingRefreshRequestedAt = new Map();
coldDetector.poolStateCache = {
  get: () => null,
  refreshOne: async () => { refreshCalls += 1; },
};
assert.strictEqual(coldDetector._getPricingState('ColdPool'), null);
assert.strictEqual(coldDetector._getPricingState('ColdPool'), null);
assert.strictEqual(refreshCalls, 1, 'cold pool pricing refresh must be throttled');
